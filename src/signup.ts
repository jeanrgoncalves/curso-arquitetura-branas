			...
			if (input.name && input.name.match(/[a-zA-Z] [a-zA-Z]+/)) {
				if (input.email && input.email.match(/^(.+)@(.+)$/)) {
					...
					if (validateCpf(input.cpf)) {
						if (input.isDriver) {
							if (input.carPlate && input.carPlate.match(/[A-Z]{3}[0-9]{4}/)) {
				...
				return res.status(422).json({ message: "Ivalid name." });
			}

		} else {
			return res.status(422).json({ message: "It already exists an account for this email." });
		}
